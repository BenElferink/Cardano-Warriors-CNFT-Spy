...
function Listings({ title = "Listings", options = {} }) {
  ...
  const handleScroll = (e) => {
    ...
    if (
      !isLoadingMore &&
      ((isDesktop && isScrolledToBottom) || (!isDesktop && isScrolledToRight))
    ) {
      pageRef.current += 1;
    ...
    >
      ...
      >
        <CardActionArea onClick={clickItem}>
          <CardMedia
            component="img"
            image={`https://ipfs.io/ipfs/${listing.asset.metadata.image.replace(
              "ipfs://",
              ""
            )}`}
            alt=""
          ...
          <CardContent>
            ...
            <Typography variant="body2" color="text.secondary">
              ...
              <span className={styles.iconsWrapper}>
                ...
                <span className={styles.iconItem}>
                  <LocalOfferIcon fontSize="small" /> {listing.offers.length}
                </span>
              </span>
            </Typography>
          </CardContent>
